...
class AvoidanceLayer(Node):
    ...
    def __init__(self):
        ...
        self.bump_counter_reduce_timer = self.create_timer(
            self.config["BUMP_COUNTER_REDUCE_TIMER"], self.bump_counter_reduce
        )
    ...
    def update_actions(self):
        ...
        if self.state == AvoidanceLayerStates.NO_COLLISION and self.bump_data:
            ...
            if self.bump_counter < self.config["MAX_BUMPS_BEFORE_AVOID"]:
                ...
                elif self.delay_counter == (
                    self.config["AVOIDANCE_DELAY"] - self.config["AVOIDANCE_STEP"]
                ):
                    self.action_publisher.publish(self.go_msg)
                elif self.delay_counter == (
                    self.config["AVOIDANCE_DELAY"] - (self.config["AVOIDANCE_STEP"] * 2)
                ):
                    self.action_publisher.publish(self.right_turn_msg)
                elif self.delay_counter == (
                    self.config["AVOIDANCE_DELAY"] - (self.config["AVOIDANCE_STEP"] * 3)
                ):
                    self.action_publisher.publish(self.go_msg)
                elif self.delay_counter == (
                    self.config["AVOIDANCE_DELAY"] - (self.config["AVOIDANCE_STEP"] * 4)
                ):
                    self.action_publisher.publish(self.right_turn_msg)
                elif self.delay_counter == (
                    self.config["AVOIDANCE_DELAY"] - (self.config["AVOIDANCE_STEP"] * 5)
                ):
                    self.action_publisher.publish(self.go_msg)
                elif self.delay_counter == (
                    self.config["AVOIDANCE_DELAY"] - (self.config["AVOIDANCE_STEP"] * 6)
                ):
                    self.action_publisher.publish(self.left_turn_msg)
                    self.bump_counter = 0
                    self.pause_bump_counter = False
