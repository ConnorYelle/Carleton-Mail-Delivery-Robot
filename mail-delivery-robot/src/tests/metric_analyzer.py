...
class MetricAnalyzer(Node):
    def __init__(self):
        ...
        self.create_subscription(
            String, "intersection_detection", self.intersection_callback, 10
        )
    ...
    def intersection_callback(self, msg):
        if msg.data == "TRUE" and self.last_camera_trigger_time:
            delay = (
                self.get_clock().now() - self.last_camera_trigger_time
            ).nanoseconds / 1e9
    ...
    def log_metrics(self):
        ...
        m2_avg_delay = (
            float(np.mean(self.intersection_delays))
            if self.intersection_delays
            else 0.0
        )
