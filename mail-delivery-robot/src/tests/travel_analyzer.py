...
class TravelAnalyzer(Node):
    ...
    def log_metrics(self):
        ...
        if self.wall_distances:
            ...
            std_dev_wall = np.std(
                [abs(d - self.target_wall_distance) for d in self.wall_distances]
            )
            self.get_logger().info(
                f"Wall Distance (Avg: {avg_wall_distance:.2f}, Std: {std_dev_wall:.2f})"
            )
    ...
    def display_results(self):
        ...
        std_dev_wall = (
            np.std([abs(d - self.target_wall_distance) for d in self.wall_distances])
            if self.wall_distances
            else 0
        )
        ...
        tk.Label(
            root, text="Wall Following Performance", font=("Arial", 14, "bold")
        ).pack(pady=10)
        tk.Label(
            root, text=f"Average Distance from Wall: {avg_wall_distance:.2f} m"
        ).pack()
