...
class BatteryMonitor(Node):
    ...
    def __init__(self):
        ...
        self.beacon_sub = self.create_subscription(
            String, "beacon_data", self.beacon_callback, 10
        )
        self.destination_sub = self.create_subscription(
            String, "destinations", self.destination_callback, 10
        )

        self.destination_pub = self.create_publisher(String, "destinations", 10)

    def beacon_callback(self, msg):
        self.last_beacon = msg.data.split(",")[
            0
        ]  # Fixed since the beacon data is like beacon,rssi
        # self.get_logger().info(f"Last beacon is: {self.last_beacon}")

    def destination_callback(self, msg):
        try:
            self.current_destination = msg.data.split(":")[
                1
            ]  # Again fixed to match format of source:destination
        except IndexError:
            self.current_destination = None
    ...
    def battery_callback(self, data):
        ...
        if (
            self.current_battery <= self.LOW_BATTERY_THRESHOLD
            ...
            self.low_battery_triggered
            and self.current_battery >= self.HIGH_BATTERY_THRESHOLD
        ):
            if self.saved_destination:
                self.publish_destination(self.saved_destination)
                self.get_logger().info(
                    f"Battery recharged, resuming delivery to: {self.saved_destination}"
                )
                self.saved_destination = None
            self.low_battery_triggered = False
