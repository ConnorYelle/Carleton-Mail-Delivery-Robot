...
class NavigationUnit_AI(Node):
    ...
    def update_navigation(self):
        ...
        self.get_logger().info(
            f"Current Direction: {self.direction}, Can Send: {self.can_send_direction}"
        )
        if self.direction is not None and self.can_send_direction:
            ...
            try:
                ...
                ai_decision = self.query_ollama(
                    self.current_beacon, self.current_destination
                )
                ...
                if ai_decision in [
                    ...
                    self.get_logger().info(
                        "Ollama returned invalid direction, falling back to map-based navigation."
                    )

            except Exception as e:
                self.get_logger().warn(
                    f"Ollama query failed: {e}. Falling back to map-based navigation."
                )
    ...
    def query_ollama(self, current_beacon: str, destination: str):
        ...
        try:
            ...
            result = graph.invoke(
                {"current_beacon": current_beacon, "destination": destination}
            )
