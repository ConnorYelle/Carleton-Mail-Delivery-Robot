...
class LidarSensor(Node):
    ...
    def calculate(self, scan):
        ...
        for i in range(count):
            ...
            # wall_following
            if (
                degree >= self.config["WALL_FOLLOW_MIN_ANGLE"]
                and degree <= self.config["WALL_FOLLOW_MAX_ANGLE"]
                and curDir < min_distance
            ):
                min_distance = curDir
                angle = degree
            ...
            ) and curDir < min_front:
                min_front = curDir
            elif (
                degree >= self.config["RIGHT_MIN_ANGLE"]
                and degree < self.config["RIGHT_MAX_ANGLE"]
                and curDir < min_right
            ):
                min_right = curDir
            elif (
                degree > self.config["LEFT_MIN_ANGLE"]
                and degree <= self.config["LEFT_MAX_ANGLE"]
                and curDir < min_left
            ):
                min_left = curDir
